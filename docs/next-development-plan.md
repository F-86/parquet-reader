...
已完成：形成设计结论，暂不引入 DataFusion。详见 [`docs/design/r2.3-datafusion-feasibility.md`](./design/r2.3-datafusion-feasibility.md)。
结论：当前 DSL + 类型化比较 + 列级最小化匹配已满足需求；DataFusion 的主要成本（依赖膨胀、异步桥接、错误映射）超过收益。建议下一步做轻量列裁剪（原生 Arrow reader 层面），不需引入新依赖。
...
- **R2.1** 已完成：类型化比较已落地；
- **R2.2** 已完成：列级最小化匹配已落地；
- **R2.3** 已完成：DataFusion 可行性调研已形成设计结论（暂不引入）；
- **R2** 整体完成。
